...
export default function SupervisorsPage() {
  ...
  async function handleToggleStatus(item: SupervisorItem) {
    ...
    try {
      ...
      showToast(
        `Approving Officer ${item.name} marked ${updatedStatus ? "ACTIVE" : "INACTIVE"}`
      );
    } catch (err) {
      // Rollback
      setSupervisors((prev) =>
  ...
  async function handleSaveSupervisor(e: React.FormEvent) {
    ...
    try {
      if (editingItem) {
        ...
        showToast(`Updated approving officer "${formData.name.toUpperCase()}"`);
      } else {
        ...
        if (!res.ok) {
          const err = await res.json();
          throw new Error(err.error || "Failed to register approving officer");
        }

        const created = await res.json();
        showToast(
          `Registered approving officer "${created.name}" with Badge ${created.badgeNumber}`
        );
      }
      ...
      setFormError(err.message || "Failed to save approving officer");
    } finally {
      setIsSaving(false);
    }
  }
  ...
  async function handleDeleteSupervisor(item: SupervisorItem) {
    if (!confirm(`Are you sure you want to remove approving officer "${item.name}"?`)) return;
    ...
    try {
      ...
      showToast(`Removed approving officer "${item.name}"`);
      await loadSupervisors();
    } catch (err: any) {
      alert(err.message || "Could not remove approving officer");
    }
  }
    ...
    <div className="space-y-6 max-w-7xl mx-auto pb-12">
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 bg-white p-6 rounded-2xl border border-slate-200 shadow-2xs">
        <div>
          ...
          <p className="text-xs text-slate-500 font-medium mt-1">
            Manage authorized vehicle inspection and booking approving officers. Badge numbers are automatically generated upon registration, and active officers appear directly in the certification select dropdown at the Booking Desk.
          </p>
        </div>
        ...
        <div className="flex items-center gap-2">
          ...
          >
            <span>+</span>
            <span>Register Approving Officer</span>
          </button>
        </div>
      </div>
      ...
      {loading ? (
        <div className="bg-white p-12 rounded-2xl border border-slate-200 text-center text-sm font-semibold text-slate-400">
          Loading approving officers from database...
        </div>
      ) : filteredSupervisors.length === 0 ? (
        <div className="bg-white p-12 rounded-2xl border border-slate-200 text-center space-y-2">
          <p className="text-sm font-bold text-slate-700">No approving officers found</p>
          <p className="text-xs text-slate-400">Try adjusting your search criteria or register a new approving officer.</p>
        </div>
      ) : (
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left text-xs text-slate-600">
              <thead className="bg-slate-50 border-b border-slate-100 text-[11px] font-extrabold text-slate-700 uppercase tracking-wider">
                <tr>
                  <th className="py-3.5 px-4">Approving Officer Name &amp; ID</th>
            ...
            <div className="flex items-center justify-between pb-3 border-b border-slate-100">
              <div className="flex items-center gap-2">
                ...
                <h3 className="text-sm font-bold text-slate-900 uppercase">
                  {editingItem ? `Edit Officer: ${editingItem.name}` : "Register Approving Officer"}
                </h3>
              </div>
              <button
                type="button"
              ...
              <div className="p-3 bg-emerald-50/70 border border-emerald-200 rounded-xl space-y-1">
                ...
                <p className="text-[11px] text-emerald-800">
                  {editingItem?.badgeNumber
                    ...
                    : "An official unique approving officer badge number (e.g. SUP-XXXX) will be automatically generated and assigned by the database upon registration."}
                </p>
              </div>
